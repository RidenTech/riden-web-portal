...
<div class="reviews-container">
    ...
    <div class="reviews-header-row riden-list-header d-flex justify-content-between align-items-center">
        ...
        <div>
            <button class="btn btn-primary px-4 fw-semibold" style="border-radius: 8px;" data-bs-toggle="modal" data-bs-target="#addDriverReviewModal">
    ...
    <div class="riden-tabs-container">
        <a href="{{ route('admin.reviews.ratings', ['tab' => 'drivers']) }}" class="riden-tab-item active">Drivers Reviews</a>
        <a href="{{ route('admin.reviews.ratings', ['tab' => 'passengers']) }}" class="riden-tab-item">Passengers Reviews</a>
    </div>
    ...
    <div class="reviews-list mt-4">
        ...
        <div class="review-item-card p-3 border rounded mb-3 shadow-sm" style="background: #fff;">
            <div class="review-item-header d-flex justify-content-between align-items-center mb-2">
                <div class="reviewer-info d-flex align-items-center">
                    <div class="rounded-circle bg-primary text-white d-flex align-items-center justify-content-center me-3" style="width: 45px; height: 45px; font-weight: bold; font-size: 18px;">
                        {{ strtoupper(substr($review->driver->first_name ?? 'D', 0, 1)) }}
                    </div>
                    <div>
                        <h5 class="reviewer-name mb-0 fw-bold">{{ $review->driver->first_name ?? 'Unknown' }} {{ $review->driver->last_name ?? 'Driver' }}</h5>
                ...
                <div class="review-meta text-end">
                    ...
                    <div class="badge bg-light text-dark border mt-1">Driver ID: {{ $review->driver_id }}</div>
                </div>
            </div>
            ...
            <div class="review-footer d-flex justify-content-between align-items-center border-top pt-2 mt-2">
                <div class="by-passenger text-muted small fw-semibold">
                    <i class="bi bi-person-fill me-1"></i> By {{ $review->reviewer_name ?? 'Admin/Passenger' }}
                </div>
        ...
        <div class="text-center p-5 border rounded" style="background: #f8f9fa;">
            ...
            <p class="text-muted">Click the "Add Review" button to create the first review for a driver.</p>
        </div>
        @endforelse
    </div>
...
<!-- Add Review Modal -->
<div class="modal fade" id="addDriverReviewModal" tabindex="-1" aria-labelledby="addDriverReviewModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg" style="border-radius: 16px;">
            <div class="modal-header border-0 pb-0">
                <h5 class="modal-title fw-bold" id="addDriverReviewModalLabel">Assign Review to Driver</h5>
            ...
            <form action="{{ route('admin.reviews.store') }}" method="POST">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label class="form-label fw-semibold text-muted small">Select Driver</label>
                        <select name="driver_id" class="form-select form-select-lg" style="font-size: 14px;" required>
                            <option value="">-- Choose a Driver --</option>
                            @foreach($drivers ?? [] as $d)
                                <option value="{{ $d->id }}">{{ $d->first_name }} {{ $d->last_name }} (ID: {{ $d->id }})</option>
                            @endforeach
                        </select>
                    </div>

                    <div class="mb-3">
                        <label class="form-label fw-semibold text-muted small">Reviewer Name (e.g. Passenger Name)</label>
                        <input type="text" name="reviewer_name" class="form-control" placeholder="John Doe" required>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="d-flex align-items-center bg-light p-2 rounded" style="gap: 15px;">
                            <input type="range" name="rating" min="1" max="5" step="0.5" value="5" class="form-range flex-grow-1" id="ratingSlider" oninput="document.getElementById('ratingOutput').innerText = this.value + ' Stars'">
                            <span id="ratingOutput" class="badge bg-warning text-dark fw-bold px-3 py-2" style="font-size: 14px;">5 Stars</span>
                        </div>
                    </div>
